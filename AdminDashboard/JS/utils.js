export function renderCarsTable(handleCar, carsTable) {
  ...
  cars.forEach((car) => {
    ...
    row.innerHTML = `
      <td>${car.car_id}</td>
      <td><img src="./Images/${car.url_img}" alt="${car.brand
...
export function renderBookingsTable(handleBook, bookingsTable) {
  ...
  bookings.forEach((booking) => {
    ...
    row.innerHTML = `
      ...
      <td>${booking ? `${booking.brand} ${booking.model}` : "Car not found"
      }</td>
      <td>${booking.firstName}</td>
      <td>${booking.pickupLocation}</td>
      <td>${booking.dropoffLocation}</td>
      <td>$${booking.totalCost || "N/A"}</td>
      <td><span class="badge ${booking.status === "Active" ? "bg-success" : "bg-primary"
      }">${booking.status || "Active"}</span></td>
      <td>

        <button class="btn btn-sm btn-danger cancel-booking" data-id="${booking.bookingId
      }">Cancel</button>
      </td>
    `;
...
function validateCarYear(year, isEdit = false) {
  ...
  if (isNaN(yearNum) || yearNum < 1900 || yearNum > currentYear + 1) {
    errorElement.textContent = `Year must be between 1900 and ${currentYear + 1
      }`;
    errorElement.classList.remove("d-none");
    return false;
  }
