...
class BookingController {
  ...
  loadBookingsFromStorage() {
    ...
    if (storedBookings) {
      return JSON.parse(storedBookings).map(
        (booking) =>
          new BookingForm(booking.pickupLocation, booking.dropoffLocation, booking.firstName, booking.lastName,
            booking.phoneNumber, booking.email, booking.carId, booking.brand, booking.model, booking.bookingId, booking.totalCost, booking.status || "Active")
      );
    }
    return [];
  }
  ...
  createBooking(pickupLocation, dropoffLocation, firstName, lastName, phoneNumber, email,
    carId, brand, model, totalCost, status = "Active") {
    const bookingId = this.generateUniqueId();
    return new BookingForm(pickupLocation, dropoffLocation, firstName, lastName, phoneNumber, email, carId,
      brand, model, bookingId, totalCost, status);
  }
